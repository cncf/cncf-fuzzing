...
func ossFuzzInit() {
	// swaggerjson is autogenerated at compile time when OSS-fuzz is running
	// the fuzzer.
	swaggerJson := swaggerjson
	_ = os.WriteFile(filepath.Join(".", "swagger.json"), []byte(swaggerJson), 0777)
	fakeResources = tst.NewFakeResources(filepath.Join(".", "swagger.json"))
}
...
func FuzzCreateElement(data []byte) int {
	// use this initter when running locally:
	// kubectlInitter.Do(localFuzzingInit)

	// use this inittier running on OSS-fuzz:
	kubectlInitter.Do(ossFuzzInit)
