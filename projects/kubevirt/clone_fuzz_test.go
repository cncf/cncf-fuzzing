...
func FuzzVMCloneController(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte, numberOfVMs, numberOfVMClones uint8) {
		...
		cf.AddFuncs([]interface{}{
			fuzztestutils.CustomObjectMetaFuzzer(metav1.NamespaceDefault),
			fuzztestutils.CustomVMFuzzer(metav1.NamespaceDefault),
			fuzztestutils.CustomVMCloneFuzzer(metav1.NamespaceDefault),
		})
		...
		for _ = range maxVMClones {
			...
			cf.GenerateStruct(vmClone)
			
			// Link clone to an existing VM
			if vmClone.Spec.Source != nil && vmClone.Spec.Source.Name == "" {
				vmClone.Spec.Source.Name = vms[len(vmClones)%len(vms)].Name
			}
