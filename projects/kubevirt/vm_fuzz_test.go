...
func FuzzExecute(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte, numberOfVMs uint8) {
		...
		for range int(numberOfVMs) % maxResources {
			...
			if err := cf.GenerateStruct(virtualMachine); err != nil {
				continue
			}
			...
			if virtualMachine.GetObjectMeta().GetName() == "" {
				name := fmt.Sprintf("vm-%d", currentName)
				currentName += 1
			...
			// Custom fuzzer should create template, but ensure it's there
			...
			cf.GenerateStruct(&setLatestAnnotation)
